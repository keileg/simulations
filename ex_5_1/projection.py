...
class ProjectionOperator():
    ...
    def dofs(self, g, dof_type):
        if dof_type=="cell":
            ...
            raise ValueError("Unknown type of dof: " + dof_type +
...
def edge_assemble(gb, keyword, name, dof_type, side, grid_name='mortar_grid',
                  transpose=False):

    if transpose:
        matrix = np.empty((gb.num_graph_edges(),
                           gb.num_graph_nodes()), dtype=np.object)
    else:
        matrix = np.empty((gb.num_graph_nodes(),
                           gb.num_graph_edges()), dtype=np.object)
